...
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\C{\$\_}\;{}\<[9]%
\>[9]{}\mathbin{:}\;(\V{c}\;\mathbin{:}\;\D{Char})\;\to \;[\mskip1.5mu \;\V{c}\;\mskip1.5mu]\;\D{\in\llbracket}\;\C{\$}\;\V{c}\;\D{\rrbracket}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
...
for a RE. As an example, value \ensuremath{\C{inl}\;\C{\epsilon}\;((\C{\$}\;\text{\tt 'a'})\;\C{\bullet}\;(\C{\$}\;\text{\tt 'b'})} denotes
...
\ColumnHook
\end{hscode}\resethooks
The next lemmas state that functions \ensuremath{\F{flat}} and \ensuremath{\F{unflat}} are inverses and they are proved 
by induction on \ensuremath{\V{t}} and \ensuremath{\V{xs}\;\D{\in\llbracket}\;\V{e}\;\D{\rrbracket}}, respectively.
\begin{Lemma}
...
register which branch was chosen in a parse tree for a union operator, \ensuremath{\C{+}}, and the beginning 
...
We represent bit sequences using lists whose elements are of type \ensuremath{\D{Bit}}. 
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{3}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
the bit \ensuremath{\C{0_b}} marks that the parse tree for \ensuremath{\V{l}\;\C{+}\;\V{r}} is built from \ensuremath{\V{l}}'s and bit \ensuremath{\C{1_b}} that it is
built from \ensuremath{\V{r}}'s. For the Kleene star, we use bit \ensuremath{\C{1_b}} to denote the parse tree for the empty string and
...
\ColumnHook
\end{hscode}\resethooks
Next we present function \ensuremath{\F{decode}} which generates a parse tree
...
As one might expect, decoding a bit sequence produced by the \ensuremath{\F{code}} function 
will produce the original parse tree.

\begin{Theorem}
   Let \ensuremath{\V{t}\;\mathbin{:}\;\D{Tree}\;\V{e}} for some RE \ensuremath{\V{e}}. Then, it holds that \ensuremath{\F{decode}\;(\F{code}\;\V{t})\;\D{\equiv}\;\V{t}}.
\end{Theorem}
%\begin{proof}
...
Type \ensuremath{\D{BitRegex}} defines the syntax of BRE, which just associates a list of bits to a BRE.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{3}{@{}>{\hspre}l<{\hspost}@{}}%
\column{10}{@{}>{\hspre}l<{\hspost}@{}}%
...
\ColumnHook
\end{hscode}\resethooks
Conversion between \ensuremath{\D{Regex}} and \ensuremath{\D{BitRegex}} types is done by functions \ensuremath{\F{internalize}} and \ensuremath{\F{erase}}.  
First, we define an auxiliary function, \ensuremath{\F{fuse}}, which  attachs a bit code to the top-most position of a BRE.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{5}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
\\
\>[5]{}\F{internalize}\;(\V{e}\;\C{+}\;\V{e'})\;{}\<[E]%
\\
\>[5]{}\hsindent{2}{}\<[7]%
\>[7]{}\mathrel{=}\;\C{choice}\;\C{\lbrack\:\rbrack}\;(\F{fuse}\;[\mskip1.5mu \;\C{0_b}\;\mskip1.5mu]\;(\F{internalize}\;\V{e}))\;{}\<[E]%
\\
\>[7]{}\hsindent{19}{}\<[26]%
\>[26]{}(\F{fuse}\;[\mskip1.5mu \;\C{1_b}\;\mskip1.5mu]\;(\F{internalize}\;\V{e'})){}\<[E]%
\\
...
which are proved by induction on the structure of \ensuremath{\V{e}}.
\begin{Lemma}\label{erase-fuse-lemma}
For all \ensuremath{\V{e}} and \ensuremath{\V{bs}}, \ensuremath{\F{erase}\;(\F{fuse}\;\V{bs}\;(\F{internalize}\;\V{e}))\;\D{\equiv}\;\V{e}}.
\end{Lemma}
%\begin{proof}
...
\ColumnHook
\end{hscode}\resethooks
Function \ensuremath{\F{\nu[ }\;\V{e}\;\mskip1.5mu]} is an immediate translation of $\nu(e)$, as
...
\subsection{Derivatives for Bit-annotated REs and its Properties}
...
parse information is built from a nullability test result using functions \ensuremath{\F{mkEps}} and \ensuremath{\F{fuse}}.
For the Kleene star operation we record the start of a new iteration fusing \ensuremath{[\mskip1.5mu \;\C{0_b}\;\mskip1.5mu]} and we mark a start of a
new matching iteration by attaching the empty list in \ensuremath{\C{star}\;\C{\lbrack\:\rbrack}\;\V{e}}.
...
on the structure of \ensuremath{\V{e}}.
